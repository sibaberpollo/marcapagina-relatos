  ...
  try {
    ...
    while ((articleMatch = articlePattern.exec(html)) !== null) {
      ...
      const titleMatch = articleHtml.match(
        /<h2[^>]*?class="ele-article__content__title"[^>]*?><a[^>]*?>(.*?)<\/a><\/h2>/
      )
      const title = titleMatch ? titleMatch[1].replace(/<[^>]*>/g, '') : ''

      // Extraer resumen
      const summaryMatch = articleHtml.match(
        /<p[^>]*?class="ele-article__content__excerpt"[^>]*?>(.*?)<\/p>/
      )
      const summary = summaryMatch ? summaryMatch[1].trim() : ''

      // Extraer categoría
      const categoryMatch = articleHtml.match(
        /<a[^>]*?class="ele-article__content__category"[^>]*?>(.*?)<\/a>/
      )
