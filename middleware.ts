...
export async function middleware(request: NextRequest) {
  ...
  const multilingualPagesWithDynamicRoutes = ['post', 'micronarrativas'] // For /post/[slug] type routes
    ...
    for (const page of multilingualPagesWithDynamicRoutes) {
      if (pathname.startsWith(`/${locale}/${page}/`)) {
        ...
        const response = NextResponse.rewrite(
          new URL(`/${page}/${dynamicPart}?lang=${locale}`, request.url)
        )
        response.headers.set('x-pathname', pathname)
        response.headers.set('x-locale', locale)
        return response
