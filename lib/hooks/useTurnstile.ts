...
export function useTurnstile(sitekey: string) {
      ...
      if (window.turnstile && captchaRef.current && widgetIdRef.current === null) {
        ...
        try {
          const onTokenCallback = (t: string) => {
            console.log(
              'Callback de Turnstile ejecutado con token:',
              t ? 'Token recibido' : 'Token vacío'
            )
  ...
  useEffect(() => {
    if (isReady && !token) {
      ...
      ) // Espera incremental entre intentos con un máximo
  ...
  const getTokenManual = () => {
    ...
    try {
      ...
      if (storedToken) {
        console.log(
          'Obteniendo token de sessionStorage:',
          storedToken ? 'Token encontrado' : 'No hay token'
        )
        setToken(storedToken)
        return storedToken
      }
